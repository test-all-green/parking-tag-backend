...
import cn.tag.AlipayProperties;
...
public class PayController {
     ...
     */
    @PostMapping()
    public void gotoPayPage(@RequestBody Map parkingOrder, HttpServletResponse response) throws AlipayApiException, IOException {
        // 订单模型
        String productCode="QUICK_WAP_WAY";
        AlipayTradeWapPayModel model = new AlipayTradeWapPayModel();
        model.setOutTradeNo(UUID.randomUUID().toString());
        model.setSubject("支付费用");
        model.setTotalAmount(String.valueOf(parkingOrder.get("money")));
        model.setBody("支付费用");
        ...
        Integer orderId = (Integer) parkingOrder.get("id");
        ParkingOrder byId = parkingOrderRepository.findById(orderId).get();
        byId.setStatus(OrderStatusEnum.FINISH.getKey());
        parkingOrderRepository.save(byId);
